...
function plotaudioOld
  ...
  c = 'lime green'
  !null = window(dim = [1300, 800], loc = [100, 50], background_color = 'black')
...
function plotaudioNew
  ...
  xaxis = axis('X', location = -1, target = p, color = c, tickdir = 1, $
    ...
    minor = 0, thick = 1)
  !null = text([3.6, 4.5, 5.5, 6.5, 7.5, 8.5, 9.5, 10.5, 11.5] / sampling, -2.2 + fltarr(9), $
    ['+1', '+0.27', '-1', '-1', '-1', '-1', '-1', '+0.5', '+1'], /data, $
    target = p, color = c, font_size = 24, font_style = 'Bold', alignment = 0.5, clip = 0)
  !null = polyline([min(t), max(t)], [0, 0], /data, target = p, linestyle = 2, color = c)
  return, !null
end
