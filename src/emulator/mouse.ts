...
const driver = new Uint8Array([
  0x60, 0x00, 0x00, 0x00, 0x00, 0x38, 0x00, 0x18, 0x00, 0x00, 0x00, 
...
let mousex: number = 0;
let mousey: number = 0;
let clampxmin: number = 0;
let clampymin: number = 0;
let clampxmax: number = 0x3ff;
let clampymax: number = 0x3ff;
let mode: number = 0x00;
let bstatus: number = 0x00;
let istatus: number = 0x00;
let lastbstatus: number = 0x00;
let lastmousex: number = 0;
let lastmousey: number = 0;

let tmpmousex: number = 0;
let tmpmousey: number = 0;

let servestatus: number = 0;
...
export const MouseCardEvent = (event: MouseEventSimple) => {
  ...
  if(mode & 0x01)
  {
    if(b>=0)
    {
      ...
      if(x>=0 && x<=1.0)
      {
        mousex = Math.round( (clampxmax-clampxmin) * x + clampxmin );
        // mark movement int
        istatus |= 0x02;
      }
      if(y>=0 && y<=1.0)
      {
        mousey = Math.round( (clampymax-clampymin) * y + clampymin );
        // mark movement int
        istatus |= 0x02;
      }
    }
