...
import { playVeraPcmWrite, playVeraPsgWrite, playVeraPsgBatch } from "./devices/audio/vera_psg_audio"
...
export const requestKeySequence = (payload: KeySequenceRequest) => {
  const timeoutMs = Number.isInteger(payload.timeoutMs)
    && payload.timeoutMs >= 1
    && payload.timeoutMs <= 120000
    ? payload.timeoutMs
    : 0
  return requestWorkerOperation<KeySequenceResult>(
    MSG_MAIN.KEY_SEQUENCE,
    payload,
    timeoutMs + 1000,
  )
}
...
export const doOnMessage = (e: MessageEvent): {speed: number, helptext: string} | null => {
  switch (e.data.msg as MSG_WORKER) {
    ...
    case MSG_WORKER.VERA_PSG_BATCH: {
      const psgBatch = e.data.payload as VeraPsgWrite[]
      playVeraPsgBatch(psgBatch)
      break
    }
