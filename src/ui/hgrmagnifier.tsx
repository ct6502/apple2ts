...
const drawBytes = (canvas: HTMLCanvasElement, pixels: number[][]) => {
  ...
  for (let i = 1; i < nRowsHgrMagnifier; i++) {
...
const HgrMagnifier = (props: MagnifyProps) => {
  ...
  // Keep using the table's location while its lock request takes effect.
  const useSavedLocation = props.lockHgrMagnifier || updateHgrMagnifier
  const displayX = (useSavedLocation && hgrMagnifierLoc[0] >= 0)
    ? Math.max(Math.min(hgrMagnifierLoc[0], 40), 0)
    : Math.max(Math.min(x, 40), 0)
  const displayY = (useSavedLocation && hgrMagnifierLoc[1] >= 0)
