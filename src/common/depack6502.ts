...
export const depack6502 = (
  ...
  const rd = (a: number): number => {
    if (a >= 0xC000 && a < 0xC100) return a === 0xC000 ? 0x80 : 0
    return mem[a]
  }
  ...
  while (cycles++ < MAX_CYCLES) {
    ...
    switch (op) {
      ...
      default:
        throw new Error(
          `depack6502: unknown opcode $${op.toString(16).padStart(2, "0")} at PC=$${((PC - 1) & 0xFFFF).toString(16).padStart(4, "0")} after ${cycles} cycles`
        )
    }
  }
