...
test("SoftCard toggle switch detection ($C0C0 for Slot 4)", () => {
  const card = new SoftCard(4)
  expect(card.isToggleSwitch(0xc0c0)).toBe(true)
  expect(card.isToggleSwitch(0xc0c1)).toBe(true)
  expect(card.isToggleSwitch(0xc0c9)).toBe(false)
...
test("SoftCard CPU toggle on read/write", () => {
  ...
  expect(card.activeCpu).toBe("6502")
  card.readByte(0xc0c0)
  expect(card.activeCpu).toBe("Z80")
  card.writeByte(0xc0c0, 0x00)
  expect(card.activeCpu).toBe("6502")
})
