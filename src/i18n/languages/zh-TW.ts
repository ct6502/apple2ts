export const zhTW = {
  ...
  "help": {
    ...
    "urlParametersBody": "address=1234 (十六進位資料的十六進位載入位址)\nappmode=game|embed (應用程式模式：遊戲|嵌入)\ncapslock=off (大寫鎖定：關閉)\ncolor=color|nofringe|green|amber|white|inverse (顏色模式)\ncrtdistort=on|off (CRT 失真效果)\ndebug=on (除錯模式)\nghosting=on|off (殘影效果)\nhex=hexadecimal program to load at address (在指定位址載入的十六進位程式)\nmachine=apple2p|apple2eu|apple2ee (機器型號：II+, 增強型 IIe, 非增強型 IIe)\nramdisk=64|512|1024|4096|8192 (RAM 磁碟大小)\nrun=false (不執行 Hex 或 BASIC 程式)\nscanlines=on|off (掃描線)\nsound=off (音效：關閉)\nspeed=snail|slow|normal|two|three|fast|warp (速度)\ntext=<a href=\"https://www.urlencoder.org\" target=\"_blank\" rel=\"noopener noreferrer\">urlencoded</a> string or BASIC program to paste (要貼上的 urlencoded 字串或 BASIC 程式)\ntheme=classic|dark|minimal (佈景主題)\ntour=main|debug|settings (導覽)\n#urltodiskimage (磁碟映像檔網址)",
