...
const readDisk = async (file: File, drive: number) => {
  ...
  handleSetDiskData(true, drive, new Uint8Array(buffer), file.name)
}

const resetDrive = (drive: number) => {
  handleSetDiskData(true, drive, new Uint8Array(), "")
}

class HardDrive extends React.Component<{drive: number}, {}> {
  ...
  render() {
    ...
    return (
      <span className="drive">
        <img className="harddrive disk2" src={img1} alt={filename}
          title={filename}
          onClick={() => {
